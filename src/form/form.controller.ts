...
export class FormController {
    ...
    @ApiResponse({ status: 201, description: 'Form was created successfully', type: Form })
    ...
    @Get()
    @Auth(ValidRoles.admin, ValidRoles.evaluator)
    @ApiResponse({ status: 200, description: 'Forms retrieved successfully', type: [Form] })
    @ApiResponse({ status: 401, description: 'Unauthorized - Access denied' })
    async findAll() {
    ...
    @Auth(ValidRoles.admin, ValidRoles.evaluator)
    @ApiParam({ name: 'classroomId', type: 'number', description: 'The ID of the classroom to retrieve form for' })
    @ApiResponse({ status: 200, description: 'Forms retrieved successfully', type: [Form] })
    @ApiResponse({ status: 401, description: 'Unauthorized - Access denied' })
    ...
    @Patch(':id')
    @Auth(ValidRoles.admin, ValidRoles.evaluator)
    @ApiParam({ name: 'id', type: 'number', description: 'The ID of the form to update' })
    @ApiResponse({ status: 200, description: 'Form updated successfully', type: Form })
    @ApiResponse({ status: 400, description: 'Bad Request - Invalid input data' })
    @ApiResponse({ status: 401, description: 'Unauthorized - Access denied' })
    @ApiResponse({ status: 404, description: 'Form not found' })
    ...
    @Delete(':id')
    @Auth(ValidRoles.admin, ValidRoles.evaluator)
    @ApiParam({ name: 'id', type: 'number', description: 'The ID of the form to delete' })
    @ApiResponse({ status: 200, description: 'Form deleted successfully', type: Form })
    @ApiResponse({ status: 401, description: 'Unauthorized - Access denied' })
    @ApiResponse({ status: 404, description: 'Form not found' })
